...
def test_warning_titles_render_jinja_delimiters_literally():
    assert any("Missing closing '%}' in statement tag" in w for w in _warnings("{% oops }"))
    assert any("Missing closing '}}' in variable tag" in w for w in _warnings("{{ oops }"))
    assert any(
        "A single '=' only assigns, and only in '{% set %}'" in w
        for w in _warnings("{% if status = 'FINAL' %}y{% endif %}")
    )
    assert any(
        "Missing closing tag like '{% endfor %}' or '{% endif %}'" in w for w in _warnings("{% set greeting %}Hello")
    )


def test_single_equals_in_a_condition_suggests_the_comparison_operator():
    warnings = _warnings("{% if status = 'FINAL' %}y{% endif %}")
...
def test_straight_quotes_are_not_reported():
    warnings = _warnings("{% if status == 'FINAL' %}y{% endif %}{{ note == \"x\" }}")

    assert not warnings


def test_leftover_token_in_a_tag_names_the_token():
    warnings = _warnings("{% if a b c %}x{% endif %}")
...
def test_hyphenated_loop_source_is_flagged():
    warnings = _warnings("{% for r in funding-rows %}{{ r.name }}{% endfor %}")
