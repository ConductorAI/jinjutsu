...
def test_unknown_filter_does_not_raise():
    ...
    assert analyze_template(parse_result(text)) == ({}, [])
...
def _variables(text: str):
    return analyze_template(parse_result(text)).variables


def _conflicts(text: str) -> list[str]:
    return [diagnostic.render() for diagnostic in analyze_template(parse_result(text)).conflicts]
