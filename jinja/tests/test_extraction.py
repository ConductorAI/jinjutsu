...
def test_comparison_against_boolean_literal_is_boolean():
    assert analyze_template("{% if sales_rep == true %}x{% endif %}").variables == {
        "sales_rep": {"type": "boolean"},
    }
    assert analyze_template("{% if sales_rep == false %}x{% endif %}").variables == {
        "sales_rep": {"type": "boolean"},
    }
    assert analyze_template("{% if sales_rep != true %}x{% endif %}").variables == {
        "sales_rep": {"type": "boolean"},
...
def test_boolean_operators_in_condition_extract_boolean_operands():
    assert analyze_template("{% if a and b %}x{% endif %}").variables == {
        "a": {"type": "boolean"},
        "b": {"type": "boolean"},
    }
    assert analyze_template("{% if not hidden %}x{% endif %}").variables == {
        "hidden": {"type": "boolean"},
    }
...
def test_with_block_locals_are_scoped():
    ...
    assert analyze_template(text).conflicts == []
    assert analyze_template(text).variables == {"obj": {"type": "string"}}


def test_with_block_reads_its_value_before_binding_the_name():
    text = "{% with a = a %}{{ a }}{% endwith %}"

    assert analyze_template(text).variables == {"a": {"type": "string"}}
...
def test_macro_and_call_block_parameters_are_scoped():
    ...
    assert analyze_template(macro).conflicts == []
    assert analyze_template(macro).variables == {"cfg": {"type": "list", "item_format": "string"}}
    assert "item" not in analyze_template(call_block).variables
...
def test_truthiness_guard_before_field_access_is_not_a_conflict():
    assert analyze_template("{% if section %}{{ section.title }}{% endif %}").conflicts == []
    assert analyze_template("{% if s.header %}{{ s.header.title }}{% endif %}").conflicts == []
    assert analyze_template("{% for r in rows %}{% if r.meta %}{{ r.meta.id }}{% endif %}{% endfor %}").conflicts == []
