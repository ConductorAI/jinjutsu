...
```
Line 2: Mismatched loop tags
  Source: {% for r in rows %}
  Found:  1 {% for %} tag(s) but 0 {% endfor %} tag(s)
  Fix:    Each {% for %} must have a corresponding {% endfor %}
  Reason: Jinja pairs each {% for %} with the next {% endfor %}, so an unpaired one swallows the rest of the template and nothing renders.
```

```
Line 2: Unexpected 'b' after the expression
  Source: {% if a b c %}x{% endif %}
  Found:  {% if a b c %}
  Fix:    Remove 'b'. Tags should only hold one expression
  Reason: Jinja stopped here with "expected token 'end of statement block', got 'b'", so the document never renders.
```
...
```
Line 2: Jinja could not parse this tag
  Source: {%tr for line in invoice.lines %}
  Found:  {%tr for line in invoice.lines %}
  Fix:    Check the tag for typos, missing quotes, or a missing operand
  Reason: Jinja stopped here with "Encountered unknown tag 'tr'.", so the document never renders.
```
